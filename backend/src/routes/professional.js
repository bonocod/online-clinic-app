...
router.post('/questions/:id/answer', auth, isProfessional, async (req, res) => {
  try {
    ...
    if (io) {
      if (categoryId) {
        io.to(`category_${categoryId}`).emit('questionAnswered', obj)
        io.to(`category_${categoryId}`).emit('question:answered', obj)
      }
      if (postId) {
        io.to(`post_${postId}`).emit('questionAnswered', obj)
        io.to(`post_${postId}`).emit('question:answered', obj)
      }
      if (askedBy) {
        io.to(`user_${askedBy}`).emit('questionAnswered', obj)
        io.to(`user_${askedBy}`).emit('question:answered', obj)
      }
      io.to('professionals').emit('questionAnswered', obj)
      io.to('professionals').emit('question:answered', obj)
    }
...
router.get('/stats', auth, isProfessional, async (req, res) => {
  try {
    ...
    if (totalAnswers > 0) {
      const totalMs = answered.reduce((acc, question) => {
        const answeredAt = question.answeredAt
          ? question.answeredAt.getTime()
          : question.createdAt.getTime()
        const createdAt = question.createdAt.getTime()
        return acc + Math.max(0, answeredAt - createdAt)
      }, 0)
