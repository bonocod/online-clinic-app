...
// New function to get all logs for the authenticated user
const getLogs = async (req, res, next) => {
  try {
    const logs = await HealthLog.find({ userId: req.user.id }).sort({ createdAt: -1 }); // latest first
    res.json({ logs });
  } catch (err) {
    next(err);
  }
...
module.exports = { createLog, getLog, getLogs };
